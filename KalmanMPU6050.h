...
class IMU
{
  public:
    ...
    */
    ...
    /**
       Gets the raw roll (X rotation) in degress from the Kalman Filter.
       As per last read() call.

       @returns The x rotation (roll) in degrees
    */
    static double getRawRollAcc();
    ...
    /**
           ...
           @returns The x rotation (roll) in degrees
    */
    static double getRawRollGyro();
    /**
       Gets the complement raw roll (X rotation) in degress from the Kalman Filter.
       As per last read() call.

       @returns The x rotation (roll) in degrees that is passed by complementary filter
    */
    static double getRawRollComplement();
    /**
  ...
  private:
    ...
    static double rollComplement; // cuva izlaz complementary filtra
    static double rollRaw;
    static double gyroRoll;
    static double gyroRollRaw;
    static double gyroYNorm;          // cuva normalizovanu vrednost ugaone brzine Y ose
                                      // jer hocemo da racunamo complementary filtar i u glavnom programu
